...
public:
  ...
  void setupAsyncFetcher(const std::string& config_str) {
    ...
    async_fetcher_ = std::make_unique<JwksAsyncFetcher>(
        ...
        stats_,
        [this](JwtVerify::JwksPtr&& jwks) { out_jwks_array_.push_back(std::move(jwks)); });
  ...
  std::vector<JwtVerify::JwksPtr> out_jwks_array_;
...
TEST_P(JwksAsyncFetcherTest, TestGoodFetch) {
  ...
  auto jwks = JwtVerify::Jwks::createFrom(PublicKey, JwtVerify::Jwks::JWKS);
...
TEST_P(JwksAsyncFetcherTest, TestGoodFetchAndRefresh) {
  ...
  auto jwks1 = JwtVerify::Jwks::createFrom(PublicKey, JwtVerify::Jwks::JWKS);
