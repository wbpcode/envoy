...
public:
  ...
  void initializeFilter(bool emit_dynamic_metadata = false) {
    filter_ = std::make_unique<TestProxyFilter>(
        "test.", *store_.rootScope(), runtime_, access_log_, fault_config_, drain_decision_, random_,
...
// Equivalent of ConcurrentQueryWithDrainClose exercising the connection-level drain path: instead of
// polling the DrainDecision, the connection is notified via onDrain() and the filter decides to
...
TEST_F(MongoProxyFilterTest, ConcurrentQueryWithConnectionLevelDrainClose) {
  TestScopedRuntime scoped_runtime;
  scoped_runtime.mergeValues({{"envoy.reloadable_features.use_connection_level_drain", "true"}});
  initializeFilter();
  ...
  filter_->onDrain(Network::ConnectionDrainEvent{time_source_.monotonicTime(),
