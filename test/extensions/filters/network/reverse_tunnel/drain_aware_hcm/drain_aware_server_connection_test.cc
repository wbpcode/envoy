...
protected:
  DrainAwareServerConnectionTest() {
    // MockTimer(dispatcher) registers itself as the next timer returned by createTimer_. The wrapper
    // creates its timer from the connection's dispatcher.
  ...
  void raiseConnectionDrain(Server::DrainStrategy strategy = Server::DrainStrategy::Immediate) {
    connection_.raiseConnectionDrain(Network::ConnectionDrainEvent{
        connection_.dispatcher_.timeSource().monotonicTime(), std::chrono::seconds(0), strategy});
  }
...
TEST_F(DrainAwareServerConnectionTest, TimerFiresDrainDetectedViaConnectionDrain) {
  auto conn = makeConnection();
  // Notify the connection it is draining (Immediate strategy => drain right away). The DrainDecision
...
TEST_F(DrainAwareServerConnectionTest,
  ...
  auto conn = std::make_unique<DrainAwareServerConnection>(
      std::move(inner_), connection_, drain_decision_, random_, [&fired]() { fired = true; });
...
TEST_F(DrainAwareServerConnectionTest, LocalDrainFiresAtMostOnce) {
  ...
  EXPECT_CALL(*timer_, enableTimer(std::chrono::milliseconds(100), _));
  auto conn = std::make_unique<DrainAwareServerConnection>(
      std::move(inner_), connection_, drain_decision_, random_, [&fired]() { ++fired; });
  conn->shutdownNotice();
  EXPECT_EQ(1, fired);
