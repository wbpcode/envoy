...
std::unique_ptr<Envoy::Formatter::JsonFormatterImpl> makeJsonFormatter(bool omit_empty = false) {
  ...
  return std::make_unique<Envoy::Formatter::JsonFormatterImpl>(JsonLogFormat, omit_empty);
}
...
// NOLINTNEXTLINE(readability-identifier-naming)
static void BM_JsonAccessLogFormatterOmitEmpty(benchmark::State& state) {
  testing::NiceMock<MockTimeSystem> time_system;

  std::unique_ptr<Envoy::TestStreamInfo> stream_info = makeStreamInfo(time_system);
  std::unique_ptr<Envoy::Formatter::JsonFormatterImpl> json_formatter = makeJsonFormatter(true);
...
BENCHMARK(BM_JsonAccessLogFormatterOmitEmpty);
