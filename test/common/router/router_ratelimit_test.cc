...
- name: www2
  ...
  ON_CALL(Const(stream_info_), route()).WillByDefault(testing::ReturnRef(route.route));
...
public:
  void setupTest(const std::string& yaml) {
    ...
    descriptors_.clear();
    stream_info_.downstream_connection_info_provider_->setRemoteAddress(default_remote_address_);
    ON_CALL(Const(stream_info_), route()).WillByDefault(testing::ReturnRef(route_copy_));
  }
  ...
  Router::RouteConstSharedPtr route_copy_{route_};
