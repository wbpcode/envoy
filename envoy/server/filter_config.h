...
struct ExtraFactoryContext {
  ...
  // Optional stats scope that is already prefixed by the stats prefix of the filter chain, such as
  // the 'http.<stat_prefix>.' scope of the HTTP connection manager. It is only set for the
  // contexts that provide one, and when it is set statsPrefixOr() is empty: the prefix lives in the
  // name of this scope instead of in the string that is handed to the factories. Factories should
  // not read it directly but go through prefixedScopeOr() and statsPrefixOr().
  OptRef<Stats::Scope> prefixed_scope = std::nullopt;
   ...
   *         of the scope.
   */
  template <class ContextType> Stats::Scope& prefixedScopeOr(ContextType& context) const {
    if (prefixed_scope.has_value()) {
      return prefixed_scope.ref();
    }
    return scopeOr(context);
  }

  /**
   * @return the prefix to prepend to the stat names that are created in prefixedScopeOr(). It is
   ...
   *         WARNING: This should be used with prefixedScopeOr() together to ensure that the
   *         stat names are correctly prefixed.
   *
   */
  const std::string& statsPrefixOr() const {
    return prefixed_scope.has_value() ? EMPTY_STRING : stats_prefix;
  }
  ...
  static ExtraFactoryContext create(FactoryContext& context, const std::string& stats_prefix) {
    ...
    // The HTTP connection manager may hand its filters a context that carries the
    // 'http.<stat_prefix>.' scope in prefixedScope(). Every other factory context has a
    // prefixedScope() of its own that has nothing to do with the stat prefix of the filters, hence
    // the type check rather than an unconditional read. The scope stands in for one prefix only,
    // so it is not handed to a filter that is created with a different one: an ECDS filter is
    // created from the very same context but with its own
    ...
    if (auto* http_context = dynamic_cast<Http::HttpFilterFactoryContext*>(&context);
      ...
      extra_context.prefixed_scope = http_context->prefixedScope();
    }
