...
void UpstreamManagerImpl::onDecodingSuccess(StreamFramePtr response) {
  ...
  const uint64_t stream_id = response->streamOptions().streamId();
...
void Filter::onDecodingSuccess(StreamFramePtr request) {
  ...
  auto iter = frame_handlers_.find(helper.frame_->streamOptions().streamId());
  if (iter == frame_handlers_.end()) {
    ...
    ENVOY_LOG(error, "generic proxy: id {} not found for stream frame",
              helper.frame_->streamOptions().streamId());
    onDecodingFailure();
    return;
  }
