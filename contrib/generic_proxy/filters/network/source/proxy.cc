...
ActiveStream::ActiveStream(Filter& parent, StreamRequestPtr request)
    : parent_(parent), request_stream_(std::move(request)),
      response_stream_end_(request_stream_->frameOptions().endStream()),
...
void ActiveStream::onStreamFrame(StreamFramePtr frame) {
...
void ActiveStream::onResponseStart(ResponsePtr response) {
  ...
  response_stream_end_ = response_stream_->frameOptions().endStream();
  parent_.stream_drain_decision_ = response_stream_->streamOptions().drainClose();
  continueEncoding();
}

void ActiveStream::onResponseFrame(StreamFramePtr frame) {
  response_stream_frames_.emplace_back(std::move(frame));
  response_stream_end_ = frame->frameOptions().endStream();
...
StreamFrameHandler* UpstreamManagerImpl::onDecodingSuccess(StreamResponsePtr response) {
  // registered_upstream_callbacks_ should be empty because after upstream connection is ready.
  ASSERT(registered_upstream_callbacks_.empty());

  uint64_t stream_id = response->streamOptions().streamId();
...
StreamFrameHandler* Filter::onDecodingSuccess(StreamRequestPtr request) {
  return newDownstreamRequest(std::move(request));
}
...
StreamFrameHandler* Filter::newDownstreamRequest(StreamRequestPtr request) {
  const bool end_stream = request->frameOptions().endStream();
  ...
  return end_stream ? nullptr : raw_stream;
}
