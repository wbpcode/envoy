...
public:
  DubboRequest(Common::Dubbo::Metadata metadata, Common::Dubbo::RpcRequestPtr request)
      : metadata_(std::move(metadata)), message_(std::move(request)) {
    ASSERT(metadata_.request());
    ASSERT(message_);
  }
  ...
  absl::string_view host() const override { return message_->service(); }
  absl::string_view path() const override { return message_->service(); }
  absl::string_view method() const override { return message_->method(); }
  void erase(absl::string_view key) override;

  // StreamFrame
  FrameFlags frameFlags() const override {
    return {StreamFlags{metadata_.requestId(), metadata_.oneway(), false, metadata_.heartbeat()},
            true};
  }

  Common::Dubbo::Metadata metadata_;
  Common::Dubbo::RpcRequestPtr message_;
};

class DubboResponse : public Response {
public:
  DubboResponse(Common::Dubbo::Metadata metadata, Common::Dubbo::RpcResponsePtr response)
      : metadata_(std::move(metadata)), message_(std::move(response)) {
    ASSERT(metadata_.response());
    ASSERT(message_);
  ...
  // StreamFrame
  FrameFlags frameFlags() const override {
    return {StreamFlags{metadata_.requestId(), false, false, metadata_.heartbeat()}, true};
  }

  StreamStatus status_;

  Common::Dubbo::Metadata metadata_;
  Common::Dubbo::RpcResponsePtr message_;
};

class DubboCodecBase : public Logger::Loggable<Logger::Id::connection> {
public:
  DubboCodecBase(Common::Dubbo::DubboCodec codec);

  Common::Dubbo::DubboCodec codec_;
};

template <class CodecType, class DecoderMessageType, class InnerDecoderMessageType,
          class EncoderMessageType, class CallBackType>
...
public:
  ...
  Common::Dubbo::DecodeStatus decodeOne(Buffer::Instance& buffer) {
    ...
    std::unique_ptr<InnerDecoderMessageType> inner_message_;
    ...
    if (decode_status == Common::Dubbo::DecodeStatus::Success) {
      ...
      decode_status = codec_.decodeData(buffer, metadata_.value(), inner_message_);
    }
    ...
    auto message =
        std::make_unique<DecoderMessageType>(metadata_.value(), std::move(inner_message_));
    metadata_.reset();
  ...
  void encode(const StreamFrame& frame, EncodingCallbacks& callbacks) override {
    ...
    ASSERT(typed_message->message_ != nullptr);

    Buffer::OwnedImpl buffer;
    codec_.encodeEntireMessage(buffer, typed_message->metadata_, *typed_message->message_);
    callbacks.onEncodingSuccess(buffer, true);
  }

  Common::Dubbo::OptMetadata metadata_;
  CallBackType* callback_{};
};

class DubboServerCodec
    : public DubboDecoderBase<ServerCodec, DubboRequest, Common::Dubbo::RpcRequest, DubboResponse,
...
class DubboClientCodec
    : public DubboDecoderBase<ClientCodec, DubboResponse, Common::Dubbo::RpcResponse, DubboRequest,
