...
#include "contrib/peer_metadata/filters/http/source/filter.h"
...
MXPropagationMethod::MXPropagationMethod(
    ...
    const envoy::extensions::filters::http::peer_metadata::v3alpha::
        PeerMetadataConfig_IstioHeaders& istio_headers)
...
std::vector<DiscoveryMethodPtr> FilterConfig::buildDiscoveryMethods(
    const Protobuf::RepeatedPtrField<
        envoy::extensions::filters::http::peer_metadata::v3alpha::PeerMetadataConfig::
  ...
  for (const auto& method : config) {
    ...
    case envoy::extensions::filters::http::peer_metadata::v3alpha::PeerMetadataConfig::
        DiscoveryMethod::MethodSpecifierCase::kWorkloadDiscovery:
      methods.push_back(
          std::make_unique<XDSMethod>(downstream, factory_context.serverFactoryContext()));
      break;
    case envoy::extensions::filters::http::peer_metadata::v3alpha::PeerMetadataConfig::
        DiscoveryMethod::MethodSpecifierCase::kIstioHeaders:
...
std::vector<PropagationMethodPtr> FilterConfig::buildPropagationMethods(
  ...
  for (const auto& method : config) {
    switch (method.method_specifier_case()) {
    case envoy::extensions::filters::http::peer_metadata::v3alpha::PeerMetadataConfig::
        PropagationMethod::MethodSpecifierCase::kIstioHeaders:
      methods.push_back(
