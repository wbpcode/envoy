...
void Filter::log(const AccessLog::HttpLogContext& context, const StreamInfo::StreamInfo&) {
  // `log` may be called multiple times with different log type
  switch (context.accessLogType()) {
  ...
  case Envoy::AccessLog::AccessLogType::DownstreamEnd: {
    ...
    if (req_ == nullptr) {
      ...
      request_headers_ = static_cast<Http::RequestOrResponseHeaderMap*>(
          const_cast<Http::RequestHeaderMap*>(&context.requestHeaders()));
    }

    state.enterLog();
    req_->phase = static_cast<int>(state.phase());
    dynamic_lib_->envoyGoFilterOnHttpLog(req_, int(context.accessLogType()));
    state.leaveLog();
  } break;
  default:
