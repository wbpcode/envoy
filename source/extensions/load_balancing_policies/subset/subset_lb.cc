...
ChildLoadBalancerCreatorLegacyImpl::ChildLoadBalancerCreatorLegacyImpl(
    const Upstream::ClusterInfo& cluster_info) {

  switch (cluster_info.lbType()) {
  case LoadBalancerType::LeastRequest:
    // The cluster info will has longer lifetime than the lb load balancer, so it is safe to
    // capture the reference here.
    creator_ = [&cluster_info](const PrioritySet& priority_set,
                               const PrioritySet* local_priority_set, Runtime::Loader& runtime,
                               Envoy::Random::RandomGenerator& random,
                               TimeSource& time_source) -> LbPair {
      LoadBalancerPtr lb = std::make_unique<LeastRequestLoadBalancer>(
          priority_set, local_priority_set, cluster_info.lbStats(), runtime, random,
          cluster_info.lbConfig(), cluster_info.lbLeastRequestConfig(), time_source);
  ...
  case LoadBalancerType::Random:
    creator_ = [&cluster_info](const PrioritySet& priority_set,
                               const PrioritySet* local_priority_set, Runtime::Loader& runtime,
                               Envoy::Random::RandomGenerator& random, TimeSource&) -> LbPair {
      LoadBalancerPtr lb = std::make_unique<RandomLoadBalancer>(priority_set, local_priority_set,
                                                                cluster_info.lbStats(), runtime,
                                                                random, cluster_info.lbConfig());
      return {nullptr, std::move(lb)};
    };
    break;
  ...
  case LoadBalancerType::RoundRobin:
    creator_ = [&cluster_info](const PrioritySet& priority_set,
                               ...
                               Envoy::Random::RandomGenerator& random,
                               TimeSource& time_source) -> LbPair {
      LoadBalancerPtr lb = std::make_unique<RoundRobinLoadBalancer>(
          priority_set, local_priority_set, cluster_info.lbStats(), runtime, random,
          cluster_info.lbConfig(), cluster_info.lbRoundRobinConfig(), time_source);
      return {nullptr, std::move(lb)};
    };
    break;
  ...
  case LoadBalancerType::RingHash:
    ...
    creator_ = [&cluster_info](const PrioritySet& priority_set, const PrioritySet*,
                               Runtime::Loader& runtime, Envoy::Random::RandomGenerator& random,
                               TimeSource&) -> LbPair {
      ThreadAwareLoadBalancerPtr lb = std::make_unique<RingHashLoadBalancer>(
  ...
  case LoadBalancerType::Maglev:
    ...
    creator_ = [&cluster_info](const PrioritySet& priority_set, const PrioritySet*,
                               Runtime::Loader& runtime, Envoy::Random::RandomGenerator& random,
                               TimeSource&) -> LbPair {
      ThreadAwareLoadBalancerPtr lb = std::make_unique<MaglevLoadBalancer>(
          priority_set, cluster_info.lbStats(), cluster_info.statsScope(), runtime, random,
  ...
  // Ensure that we have a creator at this point.
  ASSERT(creator_ != nullptr);
}
...
SubsetLoadBalancer::PrioritySubsetImpl::PrioritySubsetImpl(const SubsetLoadBalancer& subset_lb,
  ...
  auto lb_pair = subset_lb.child_lb_creator_->createLoadBalancer(
      *this, subset_lb.original_local_priority_set_, subset_lb.runtime_, subset_lb.random_,
