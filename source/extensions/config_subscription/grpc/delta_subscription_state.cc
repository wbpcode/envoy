...
DeltaSubscriptionState::getNextRequestAckless() {
  ...
  if (!any_request_sent_yet_in_current_stream_) {
    ...
    // A pure wildcard subscription ("*" and nothing else) is encoded as an empty resource list on
    // the wire; never send "*" itself. A wildcard combined with explicitly-requested resources must
    // still send "*", so only clear when "*" is the sole entry to subscribe.
