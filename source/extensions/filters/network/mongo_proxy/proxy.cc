...
ProxyFilter::ProxyFilter(const std::string& stat_prefix, Stats::Scope& scope,
    ...
    : stats_(generateStats(stat_prefix, scope)), runtime_(runtime), drain_decision_(drain_decision),
      random_(random), access_log_(access_log), fault_config_(fault_config),
      time_source_(time_source),
...
bool ProxyFilter::shouldDrainClose() {
  // When connection-level drain is enabled, decide from the drain event delivered via onDrain()
  // instead of polling the DrainDecision.
  if (Runtime::runtimeFeatureEnabled("envoy.reloadable_features.use_connection_level_drain")) {
    return connection_drain_event_.has_value() &&
           Network::shouldDrainClose(time_source_, random_, *connection_drain_event_);
  }
