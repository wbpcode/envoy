...
uint64_t envoy_dynamic_module_callback_http_get_buffer_limit(
    envoy_dynamic_module_type_http_filter_envoy_ptr filter_envoy_ptr) {
  auto* filter = static_cast<DynamicModuleHttpFilter*>(filter_envoy_ptr);
  auto* callbacks = filter->decoder_callbacks_;
  if (callbacks == nullptr) {
    return 0;
  }
  return callbacks->decoderBufferLimit();
}
...
void envoy_dynamic_module_callback_http_set_buffer_limit(
  ...
  callbacks->setDecoderBufferLimit(limit);
}
...
void envoy_dynamic_module_callback_http_filter_send_go_away_and_close(
    envoy_dynamic_module_type_http_filter_envoy_ptr filter_envoy_ptr, bool) {
  auto* filter = static_cast<DynamicModuleHttpFilter*>(filter_envoy_ptr);
  if (filter->decoder_callbacks_ == nullptr) {
    return;
  }
  filter->decoder_callbacks_->sendGoAwayAndClose();
}
