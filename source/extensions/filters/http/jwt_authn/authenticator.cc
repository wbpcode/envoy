...
#include "source/extensions/filters/http/common/jwt/verify.h"
...
void AuthenticatorImpl::onJwksSuccess(JwtVerify::JwksPtr&& jwks) {
...
void AuthenticatorImpl::doneWithStatus(const Status& status) {
  ...
  if (Status::Ok != status) {
    ...
    if (!failed_status_in_metadata.empty()) {
      ...
      failed_status_fields["message"].set_string_value(JwtVerify::getStatusString(status));
