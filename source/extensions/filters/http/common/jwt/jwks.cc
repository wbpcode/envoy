...
#include "source/extensions/filters/http/common/jwt/struct_utils.h"
...
public:
  ...
  bssl::UniquePtr<EC_KEY> createEcKeyFromJwkEC(int nid, const std::string& x, const std::string& y) {
...
private:
  bssl::UniquePtr<BIGNUM> createBigNumFromBase64UrlString(const std::string& s) {
    ...
    return bssl::UniquePtr<BIGNUM>(
        BN_bin2bn(castToUChar(s_decoded), s_decoded.length(), nullptr));
  };
};
...
Status extractJwkFromJwkRSA(const ::google::protobuf::Struct& jwk_pb, Jwks::Pubkey* jwk) {
  ...
  KeyGetter e_getter;
  jwk->rsa_ = e_getter.createRsaFromJwk(n_str, e_str);
  return e_getter.getStatus();
}
...
Status extractJwkFromJwkEC(const ::google::protobuf::Struct& jwk_pb, Jwks::Pubkey* jwk) {
  ...
  KeyGetter e_getter;
  jwk->ec_key_ = e_getter.createEcKeyFromJwkEC(nid, x_str, y_str);
  return e_getter.getStatus();
}
...
Status extractJwkFromJwkOKP(const ::google::protobuf::Struct& jwk_pb, Jwks::Pubkey* jwk) {
  ...
  KeyGetter e_getter;
  jwk->okp_key_raw_ = e_getter.createRawKeyFromJwkOKP(nid, keylen, x_str);
  return e_getter.getStatus();
}
...
Status createFromX509(const ::google::protobuf::Struct& jwks_pb, std::vector<Jwks::PubkeyPtr>& keys) {
  for (const auto& kid : jwks_pb.fields()) {
...
Status Jwks::addKeyFromPem(const std::string& pkey, const std::string& kid, const std::string& alg) {
...
void Jwks::createFromJwksCore(const std::string& jwks_json) {
  ...
  for (const auto& key_value : keys_it->second.list_value().values()) {
    ...
    PubkeyPtr key_ptr(new Pubkey());
    Status extract_status = extractJwk(key_value.struct_value(), key_ptr.get());
    if (extract_status == Status::Ok) {
      keys_.push_back(std::move(key_ptr));
      resetStatus(extract_status);
    } else {
      updateStatus(extract_status);
    }
  }
