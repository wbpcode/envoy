...
absl::StatusOr<FilterFactoriesVector> createFilterFactoriesFromConfig(
  ...
  for (const auto& filter_config : proto_config.filters()) {
    ...
    if (factory.isUnifiedFilter() || !extra_context.factory_context.has_value()) {
      // If the filter has implemented the unified factory interface, or this filter is configured
      // in a route configuration (no factory context available).
      ...
      callback = callback_or_error.value();
    } else {
      auto callback_or_error = factory.createFilterFactoryFromProto(
          *message, extra_context.stats_prefix, *extra_context.factory_context);
      RETURN_IF_NOT_OK_REF(callback_or_error.status());
      callback = callback_or_error.value();
    }
    ...
    auto filter_config_provider =
        Http::FilterChainUtility::createSingletonDownstreamFilterConfigProviderManager(context)
            ->createStaticFilterConfigProvider(std::move(callback), filter_config.name());
