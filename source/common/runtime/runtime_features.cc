...
// 'http.<stat_prefix>.' as the prefixed scope of their factory context, so that the stats prefix
// they read is empty, instead of passing the 'http.<stat_prefix>.' prefix as a string and relying
// on every filter to prepend it to its stat names itself.
