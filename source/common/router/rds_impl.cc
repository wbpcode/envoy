...
void RdsRouteConfigSubscription::updateOnDemand(const std::string& aliases) {
  config_update_info_->updateOnDemand(aliases);
}
...
RdsRouteConfigProviderImpl::RdsRouteConfigProviderImpl(
  ...
  // The receiver owns the VHDS subscription, which publishes to this provider directly. It is
  // bound here rather than at construction because the receiver is created before the provider.
  config_update_info_->routeConfigProvider() = this;
}
...
RouteConfigProviderSharedPtr RdsFactoryImpl::createRdsRouteConfigProvider(
  ...
  auto provider = manager.addDynamicProvider(
      ...
      [&factory_context, &rds, &stat_prefix, &manager, &proto_traits](uint64_t manager_identifier) {
        // The VHDS subscription that the receiver may create reports its stats under the same
        // prefix that the RDS subscription uses.
