...
public:
  ...
  private:
    ...
    uint8_t inline_[InlineCapacity];
    StoragePtr stack_;
    size_t size_ = 0;
  };
   ...
   *
   * Use join() or StatNameJoiner when the joined bytes must outlive the joining expression or
...
StatName SymbolTable::InlineStorage::statName() const {
  // size_ is 0 only before anything has been assembled; any assembled name, empty included,
  // occupies at least its size-prefix byte.
  if (size_ == 0) {
    return {};
  }
  return StatName(stack_ != nullptr ? stack_.get() : inline_);
}
...
public:
  ...
  StatNameJoiner(StatNameJoiner&& other) noexcept
      : storage_(std::move(other.storage_)), sole_name_(other.sole_name_) {
    other.reset();
  }
  StatNameJoiner& operator=(StatNameJoiner&& other) noexcept {
    if (this != &other) {
      storage_ = std::move(other.storage_);
      sole_name_ = other.sole_name_;
      other.reset();
    }
    return *this;
  }
  ...
  StatName statName() const { return joined_ ? storage_.statName() : sole_name_; }
...
private:
  ...
  // The name an elided join references, in the caller's storage; empty when storage_ holds the
  // joined bytes.
  StatName sole_name_;
  bool joined_{false};
};
