...
/**
 * A from-scratch implementation of CodeStats that attaches the response code, its class, the
 * virtual host, the virtual cluster and the route to the stats it creates as explicit tags,
 * rather than leaving them to be recovered from the stat name by the tag extraction rules.
 *
 * The flat names it creates are the same as the ones CodeStatsImpl creates, so a server switching
 * between the two renames no stat. See ContextImpl::setUseExplicitTags() for how the server picks
 * between them.
 */
