...
// The object will only be accessed by the main thread. For xdstp-based and ADS
// config sources it is a singleton object that is used by all the filters that
// need to access od-cds over these config sources, and will only be allocated
// for the first occurrence of the filter. For other config sources, an object
// is allocated per config source.
...
XdstpOdCdsApiImpl::XdstpOdCdsApiImpl(const envoy::config::core::v3::ConfigSource& config_source,
  ...
  switch (config_source.config_source_specifier_case()) {
  case envoy::config::core::v3::ConfigSource::ConfigSourceSpecifierCase::
  ...
  case envoy::config::core::v3::ConfigSource::ConfigSourceSpecifierCase::kAds:
    config_source_ = staticAdsConfigSource();
    break;
  default:
    // A regular (non-ADS) config source. The subscriptions made over it are specific to this
    // config source, so they are not shared with other config sources through the singleton
    // subscriptions manager. Note that the cluster manager already shares a single OdCdsApi
    // instance among all the users of the same config source.
  ...
  if (subscriptions_manager_ == nullptr) {
    ...
    // over xdstp-based or ADS config sources.
