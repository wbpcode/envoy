...
void JsonFormatBuilder::formatValueToFormatElements(const ProtoList& list_value) {
  serializer_.addPiece(Json::Constants::ArrayBeg); // Delimiter to start list.
  for (int i = 0; i < list_value.size(); ++i) {
    if (i > 0) {
      serializer_.addPiece(Json::Constants::Comma); // Delimiter to separate list elements.
    }
    formatValueToFormatElements(list_value[i]);
  }
  serializer_.addPiece(Json::Constants::ArrayEnd); // Delimiter to end list.
}
...
void JsonFormatBuilder::formatValueToFormatElements(const ProtoDict& dict_value) {
  ...
  serializer_.addPiece(Json::Constants::MapBeg); // Delimiter to start map.
  for (size_t i = 0; i < sorted_fields.size(); ++i) {
    if (i > 0) {
      serializer_.addPiece(Json::Constants::Comma); // Delimiter to separate map elements.
    }
    // Add the key.
    serializer_.addString(sorted_fields[i].first, Json::Constants::Quote, R"(":)");

    formatValueToFormatElements(sorted_fields[i].second->second);
  }
  serializer_.addPiece(Json::Constants::MapEnd); // Delimiter to end map.
}
