...
public:
  ...
  SystemTimeFormatter(absl::string_view format, TimeFieldExtractorPtr f);
...
public:
  StreamInfoFormatterBase(absl::string_view command, absl::string_view command_arg = {},
    ...
    THROW_IF_NOT_OK(
        CommandSyntaxChecker::verifySyntax((*it).second.first, command, command_arg, max_length));

    // Create a pointer to the formatter by calling a function
    // associated with formatter's name.
    formatter_ = (*it).second.second(command_arg, max_length);
  }
