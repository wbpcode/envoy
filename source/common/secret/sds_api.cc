...
Stats::ScopeSharedPtr createSdsScope(Stats::Store& stats, absl::string_view resource_name) {
  if (Runtime::runtimeFeatureEnabled("envoy.reloadable_features.no_stats_tag_extraction")) {
    return stats.rootScope()->createScope({
        ...
        Stats::StatElementView{
            .value_ = resource_name,
            .name_ = Config::TagNames::get().XDS_RESOURCE_NAME,
            .ignore_name_ = true,
        },
    });
  }
  return stats.createScope(absl::StrCat("sds.", resource_name, "."));
}
...
SdsApi::SdsApi(envoy::config::core::v3::ConfigSource sds_config, absl::string_view sds_config_name,
    ...
    : Envoy::Config::SubscriptionBase<envoy::extensions::transport_sockets::tls::v3::Secret>(
      ...
      scope_(createSdsScope(stats, std::string(sds_config_name))),
